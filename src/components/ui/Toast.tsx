...
import { ANIMATIONS, UI_PATTERNS, TYPOGRAPHY } from "@/design/tokens"
...
>(({ className, ...props }, ref) => {
  return (
    <ToastPrimitive.Root
      ref={ref}
      className={cn(
        "group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-lg border bg-background-base p-6 shadow-lg transition-all",
...
>(({ className, ...props }, ref) => (
  <div
    ref={ref}
    className={cn(TYPOGRAPHY.headings.h4, "mb-1", className)}
    {...props}
  />
))
...
>(({ className, ...props }, ref) => (
  <div
    ref={ref}
    className={cn(TYPOGRAPHY.body.base, "opacity-90", className)}
    {...props}
  />
))
